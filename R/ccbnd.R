ccbnd <-
function (pares, r, b, vlt, cx, lwd, ecol, bw, alfa, fds, flgcx, 
    flgcr, hds, n) 
{
    ...
    if (isTRUE(flgcx == TRUE) == TRUE) {
        ...
        ifelse(isTRUE(mean(lwd) > 2L) == TRUE, lc <- (max(lwd) * 
            0.1), lc <- 0)
    ...
    for (i in 1:r) {
        ...
        else if (isTRUE(b[i] %in% multiplex::men(b)[1]) == FALSE) {
            graphics::arrows(rrot[which(seq_len(nrow(dat))%%2L == 
                1L)[i], 1], rrot[which(seq_len(nrow(dat))%%2L == 
                1L)[i], 2], rrot[which(seq_len(nrow(dat))%%2L == 
                0L)[i], 1], rrot[which(seq_len(nrow(dat))%%2L == 
        ...
        if (isTRUE(n < 15) == TRUE) {
            Hd <- data.frame(x = c((-0.6 - (0.01538462 * n)), 
                (-0.35 - (0.01538462 * n)), (-0.6 - (0.01538462 * 
                  n)), (0.4 - (0.01538462 * n))), y = c(-0.5, 
                0, 0.5, 0)) * (hds)
        }
        else {
            Hd <- data.frame(x = c(-0.7, -0.45, -0.7, 0.3), y = c(-0.5, 
                0, 0.5, 0)) * (hds)
        }
        ...
        if (isTRUE(i %in% flgcr) == TRUE) {
            prx1 <- rrot[which(seq_len(nrow(dat))%%2L == 1L)[i], 
                1]
            pry1 <- rrot[which(seq_len(nrow(dat))%%2L == 1L)[i], 
                2]
            ...
            prx2 <- rrot[which(seq_len(nrow(dat))%%2L == 0L)[i], 
                1]
            pry2 <- rrot[which(seq_len(nrow(dat))%%2L == 0L)[i], 
                2]
            ...
            if (isTRUE(b[i] %in% multiplex::men(b)[1]) == FALSE) {
                prx <- rrot[which(seq_len(nrow(dat))%%2L == 1L)[i], 
                  1]
                pry <- rrot[which(seq_len(nrow(dat))%%2L == 1L)[i], 
                  2]
                hd <- xyrt((Hd), (as.numeric(angp) - 180L))
            }
            else if (isTRUE(b[i] %in% multiplex::men(b)[1]) == 
                TRUE) {
                prx <- rrot[which(seq_len(nrow(dat))%%2L == 0L)[i], 
                  1]
                pry <- rrot[which(seq_len(nrow(dat))%%2L == 0L)[i], 
                  2]
